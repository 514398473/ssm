 ...
 */
package com.xz.annotation.utils;
...
public class ScanPackages {
	...
	private final static Logger LOGGER = LoggerFactory.getLogger(ScanPackages.class);
	 ...
	 * @param annotation 注解类
	 */
	public static Set<String> findPackageAnnotationField(String scanPackages, Class<? extends Annotation> annotation) {
		if (StringUtils.isEmpty(scanPackages)) {
			return Sets.newHashSet();
		}
		...
		for (String basePackage : packages) {
			...
			try {
				...
				for (Resource resource : resources) {
					...
					String fullClassName = loadClassName(metadataReaderFactory, resource);
					// 利用反射，根据类名获取类的所有字段
					Field[] declaredFields = Class.forName(fullClassName).getDeclaredFields();
					if (isAnnotationField(declaredFields, annotation)) {
						fullClazzSet.add(fullClassName);
					}
				}
			} catch (Exception e) {
	 ...
	 */
	private static boolean isAnnotationClass(String fullClassName, Class<? extends Annotation> annotationClass) throws ClassNotFoundException {
		// 利用反射，根据类名获取类的全部信息
	 ...
	 * @throws ClassNotFoundException
	 */
	private static boolean isAnnotationField(Field[] declaredFields, Class<? extends Annotation> annotationClass) throws ClassNotFoundException {
		// 判断字段上是否有指定注解
