...
public class QRCodeUtil {
	 ...
	 * @return
	 */
	public static String getLogoQRCode(String qrUrl, String path, String productName) {
		try {
			QRCodeUtil qrCodeUtil = new QRCodeUtil();
			BufferedImage bim = qrCodeUtil.getQR_CODEBufferedImage(qrUrl, BarcodeFormat.QR_CODE, 400, 400, qrCodeUtil.getDecodeHintType());
			return qrCodeUtil.addLogo_QRCode(bim, new File(path), new LogoConfig(), productName);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public String addLogo_QRCode(BufferedImage bim, File logoPic, LogoConfig logoConfig, String productName) {
		try {
			...
			int widthLogo = logo.getWidth(null) > image.getWidth() * 3 / 10 ? (image.getWidth() * 3 / 10) : logo.getWidth(null), heightLogo = logo
					.getHeight(null) > image.getHeight() * 3 / 10 ? (image.getHeight() * 3 / 10) : logo.getWidth(null);
			...
			// File("C:/Users/luoguohui/Desktop/TDC-" + new Date().getTime() +
			// "test.png")); 
